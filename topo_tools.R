...
elec_locs <- read_excel(path = "../data/0-global-vars/elec_pos.xlsx") %>%
  mutate(
...
topo_interp <- function(data = data, dv = dv, gridRes = 67){
...
topo_plot <- function(interp_data = interp_data, orig_data = orig_data, dv = dv, min = min, max = max){
...
group_topo <- function(interp_data = data1, orig_data = data2, freqs = x, dv = y, unit = z){
